...
const ALIYUN_CHAT_API_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";
...
interface AliyunChatCompletionRequest {
  model: string;
...
interface AliyunChatCompletionResponse {
  id: string;
  object: string;
  created: number;
  model: string;
...
async function chatCompletionRequest(
  request: AliyunChatCompletionRequest
): Promise<AliyunChatCompletionResponse> {
  ...
  try {
    const response = await fetch(ALIYUN_CHAT_API_URL, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      const errorData = (await response.json()) as AliyunErrorResponse;
      throw new AIGenerationError(
        errorData.error?.message || errorData.message || `API 请求失败: ${response.status}`,
        "aliyun-chat",
        errorData
      );
    }

    return (await response.json()) as AliyunChatCompletionResponse;
  } catch (error) {
    clearTimeout(timeoutId);
...
/**
 * 调用阿里云通义千问 Chat Completion API（带重试）
 */
export async function chatCompletion(
  ...
  const request: AliyunChatCompletionRequest = {
    model: options.model || process.env.ALIYUN_CHAT_MODEL || DEFAULT_MODEL,
    messages,
